...
class CartViewModel : ViewModel() {
    ...
    val isCheckoutSuccess: StateFlow<Boolean> = _isCheckoutSuccess
    ...
    fun checkoutCart() {
        ...
        // Ubah ke format TransactionProduct
        ...
        createTransaction(
            products = products,
            onSuccess = {
                // Jika sukses, baru kosongkan cart
                clearCart(
                    cartId = currentCartId,
                    onSuccess = {
                        _cartProducts.value = emptyList()
                        _isCheckoutSuccess.value = true
                        Log.d("CHECKOUT", "Checkout berhasil dan transaksi tercatat")
                    },
                    onError = { error ->
                        _errorMessage.value = error
                        _isCheckoutSuccess.value = false
                        Log.e("CHECKOUT", "Transaksi masuk, tapi gagal kosongkan cart: $error")
                    }
                )
            },
            onError = { error ->
                _errorMessage.value = error
                _isCheckoutSuccess.value = false
                Log.e("CHECKOUT", "Gagal membuat transaksi: $error")
            }
        )
    }
